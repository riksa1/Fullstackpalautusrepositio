...
blogRouter.get("/:id", async (request, response, next) => {
  ...
  try {
    const blog = await Blog.find({ _id: id }).populate("user", "-blogs").populate("comments");
    response.json(blog);
  } catch (error) {
    next(error);
  }
...
blogRouter.delete("/:id", userExtractor, async (request, response, next) => {
  ...
  try {
    ...
    if (blog.user.toString() === user.id.toString()) {
      ...
      return response.status(401).json({ error: "you dont have the rights to remove this blog" });
    }
  } catch (error) {
    next(error);
  }
