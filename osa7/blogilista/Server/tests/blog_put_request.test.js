...
describe("api put tests", () => {
  beforeEach(async () => {
    ...
    await api.post("/api/users").send({ name: "Riko", username: "rikoo", password: "password" });
    const result = await api.post("/login").send({ username: "rikoo", password: "password" });
  ...
  test("blog is updated successfully and return right object", async () => {
    ...
    const result2 = await api.post("/login").send({ username: "rikoo", password: "password" });
    ...
    const updatedBlog = await api
      ...
      .expect(203);
    const expectedIdArray = [];
    for (var i = 0; i < result.length; i++) {
      expectedIdArray.push(result[i].id);
    }
